...
async def offer_score(offer, serverId):
    """Routes to appropriate formula based on server setting"""
    settings = serversList[str(serverId)]
    
    # Check if dynamic FA is enabled
    if settings.get('dynamicfa', 'off') == 'on':
        return await calculate_dynamic_score(offer, serverId)
    else:
        return await calculate_classic_score(offer, serverId)

async def calculate_classic_score(offer, serverId):
    """Original formula - unchanged"""
    serverExport = shared_info.serverExports.get(str(serverId))
    if serverExport is None:
        return 0
...
async def calculate_dynamic_score(offer, serverId):
    """Enhanced formula with realistic NBA-like behavior"""
    ...
    pos = player['ratings'][-1]['pos']
    asking_price = player['contract']['amount'] / 1000
    
    # Get mood trait weights
    winning_weight = float(serversList[str(serverId)]['winning'])
    fame_weight = float(serversList[str(serverId)]['fame'])
    loyalty_weight = float(serversList[str(serverId)]['loyalty'])
    money_weight = float(serversList[str(serverId)]['money'])
    
    if 'W' in player.get('moodTraits', []): winning_weight += 1
    if 'F' in player.get('moodTraits', []): fame_weight += 1
    if 'L' in player.get('moodTraits', []): loyalty_weight += 1
    if '$' in player.get('moodTraits', []): money_weight += 1
    
    # === AGE-BASED ADJUSTMENTS ===
    if age < 25:  # Young - building phase
        fame_weight *= 1.2
        if ovr < 65:
            money_weight *= 0.9  # Less money-focused, more opportunity
        # Stars want the long max, role players want flexibility
        ideal_years = 2 if ovr < 60 else (4 if ovr >= 70 else 3)
        discount_rate = 0.02
        
    elif age <= 28:  # Prime (25-28)
        money_weight *= 1.2  # Peak earning years
        winning_weight *= 1.1
        ideal_years = 4 if ovr >= 65 else 3
        discount_rate = 0.03
        
    elif age <= 31:  # Early decline (29-31)
        winning_weight *= 1.3
        loyalty_weight *= 1.1
        ideal_years = 3
        discount_rate = 0.05
        ...
        loyalty_weight *= 1.3
        winning_weight *= 1.2
        ideal_years = 2 if age < 35 else 1
        discount_rate = 0.08

    # === MONEY INFLUENCE FLOOR ===
    # In the NBA, money dominates decisions for good players.
    # Ensure money_weight is at least proportional to non-money weights.
    non_money_total = winning_weight + fame_weight + loyalty_weight
    if ovr >= 70:  # Superstars: money ~80% of decision
        money_weight = max(money_weight, non_money_total * 4.0)
    elif ovr >= 65:  # All-Stars: money ~70% of decision
        money_weight = max(money_weight, non_money_total * 2.33)
    elif ovr >= 55:  # Solid players: money ~60% of decision
        money_weight = max(money_weight, non_money_total * 1.5)

    # === CALCULATE SCORES WITHOUT MONEY MULTIPLICATION ===
    
    # 1. WINNING SCORE (NOT multiplied by money)
    team_win_pct = 0.5
    team_hype = 0.5
    for t in teams:
        if t['tid'] == offer['team']:
            if len(t['seasons']) > 0:
                ...
                team_win_pct = last_season['won'] / (last_season['won'] + last_season['lost'] + 0.0001)
                team_hype = last_season.get('hype', 0.5)
            break
    
    # Scale 0-100 instead of multiplying by offer amount
    winning_score = (team_win_pct * 0.85 + team_hype * 0.15) * 100
    
    # 2. FAME SCORE (playing time opportunity, NOT multiplied by money)
    players_better = 0
    for p2 in players:
        if p2.get('tid') == offer['team'] and p2['ratings'][-1]['ovr'] >= ovr:
            if p2['ratings'][-1]['pos'] == pos:
                players_better += 1.5
            else:
                players_better += 0.75
    
    playing_time_factor = max(0.2, 1.5 - players_better/10)
    fame_score = playing_time_factor * 100
    
    # 3. LOYALTY SCORE (NOT multiplied by money)
    years_with_team = 0
    for s in player.get('stats', []):
        if s.get('tid') == offer['team'] and not s.get('playoffs', False):
            years_with_team += 1
    
    penalty = pull_info.trade_penalty(offer['team'], serverExport)
    loyalty_score = ((1 + years_with_team/10) * 0.20 + (1 - penalty) * 0.80) * 100
    
    # 4. MONEY SCORE (NPV-based)
    # Calculate NPV
    npv = 0
    for year in range(1, offer['years'] + 1):
        npv += offer['amount'] / ((1 + discount_rate) ** year)
    
    # Compare to asking price
    money_ratio = npv / (asking_price * ideal_years) if asking_price > 0 else 1
    money_score = money_ratio * 100
    
    # === TIER-BASED ADJUSTMENTS ===
    if ovr >= 70:  # Superstar
        # Won't accept lowballs
        if offer['amount'] < asking_price * 0.9:
            money_score *= 0.5
        # Won't take 1-year without huge overpay
        if offer['years'] == 1 and age < 32:
            if offer['amount'] < asking_price * 1.5:
                return -1000  # Reject outright
                
    elif ovr >= 65:  # All-Star
        if offer['amount'] < asking_price * 0.85:
            money_score *= 0.7
            
    elif ovr < 55:  # Deep bench
        # Happy with any offer above minimum
        if offer['amount'] > serverExport['gameAttributes']['minContract'] / 1000:
            money_score *= 1.2
    
    # === CONTRACT LENGTH PREFERENCE ===
    years_diff = abs(offer['years'] - ideal_years)
    
    # Age-based year preferences
    if age >= 32:
        # Old players desperate for multi-year security
        if offer['years'] == 1:
            years_multiplier = 0.6
        elif offer['years'] >= ideal_years:
            years_multiplier = 1.2
        else:
            years_multiplier = 1 / (1 + 0.1 * years_diff)
    else:
        # Normal year preferences
        years_multiplier = 1 / (1 + 0.15 * years_diff)
    
    # === SPECIAL: Fix 1-year max spam ===
    if offer['years'] == 1 and age >= 26 and age <= 30:
        # Prime players should NOT take 1-year deals
        if ovr >= 65:  # All-stars and above
            years_multiplier *= 0.5  # Heavy penalty
    
    # === COMBINE SCORES ===
    # Each component contributes independently based on weight
    total_weight = winning_weight + fame_weight + loyalty_weight + money_weight

    if total_weight == 0:
        total_weight = 1

    # Apply years_multiplier only to non-money components
    # (NPV already accounts for contract length in money evaluation)
    non_money_score = (
        (winning_score * winning_weight) +
        (fame_score * fame_weight) +
        (loyalty_score * loyalty_weight)
    ) / total_weight

    money_component = (money_score * money_weight) / total_weight

    final_score = (non_money_score * years_multiplier) + money_component
    
    # === IDIOSYNCRATIC (for randomness) ===
    string = "no team"
    for t in teams:
        if t['tid'] == offer['team']:
            string = t['abbrev'] + t['region'] + t['name'] + "-----" + player['firstName'] + " " + player['lastName']
            break
    ...
    idiosyncratic_factor = ((integer2 % 10000) / 10000) * 0.2 + 0.9  # 0.9 to 1.1
    idiosyncratic_weight = float(serversList[str(serverId)].get('idiosyncratic', 0.05))
    
    if idiosyncratic_weight > 0:
        final_score *= (1 - idiosyncratic_weight) + (idiosyncratic_factor * idiosyncratic_weight)
    
    # Handle options if they exist (backward compatibility)
    if offer.get('option'):
        if offer['option'] == 'PO':
            final_score *= 1.15
        elif offer['option'] == 'TO':
            final_score *= 0.85
    ...
    for p in players:
        if p['pid'] == playerId:
            ...
            for v in values.values():
                if v < criticalvalue:
                    ct += 1
            # values can be empty when no players qualified in getvalues
            # (mid-season import, ratingless leagues). Skip dynamic
            # pricing in that case and just keep the existing basePrice.
            if not values:
                fraction = 0.5
            else:
            ...
            if basePrice == serverExport['gameAttributes']['minContract']:
                ...
                score = await calculate_classic_score(offer, serverId)
                #average
                total = 0
                num = 0
                for t in teams:
                    if t['disabled'] == False:
                        ...
                        total += await calculate_classic_score(offer, serverId)

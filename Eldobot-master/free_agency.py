...
commandFuncs = {
    ...
    'autofa':fa_commands.autofa,
    'bulkoffermins':fa_commands.bulkoffermins,
...
async def process_text(text, message):
    ...
    if embed is None:
        # Some command handlers return None on early-exit branches; rebuild a
        # minimal embed so embed.set_footer doesn't blow up the dispatcher.
        embed = discord.Embed(title=t['name'] + ' FA', description=f"{season} season", color=t['color'])

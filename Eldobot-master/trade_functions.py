...
async def scan_text(text, message):
    ...
    if len(text) < secondIndex+1:
        ...
        try:
            ...
            return
        if role is None:
            errorText = f"**{message.guild.name} Trade Confirmation Channel**" + '\n' + f"> {message.content}" + '\n' + '\n' + 'One of the pinged roles does not exist in this server. Make sure both mentions are valid team roles.'
            try:
                await message.delete()
            except Exception:
                pass
        ...
        if foundTeams < 2:
                ...
                for a in assets:
                    ...
                    if 'nothing' not in str.lower(a) and len(a)>1:
                        ...
                        if a.lower().startswith("for "):
                            a = a[4:]
                        if a.lower().startswith("trade ") or a.lower().startswith("traded "):
                            a = a.split(' ', 1)[1]
            ...
            if len(errors) > 0:
                ...
                players = export['players']
                teams = export['teams']
                events = export.setdefault('events', [])
                ...
                if errors != []:
                    ...
                    for t in teams:
                        if t['tid'] in confirmTeams:
                            teamsText += ' ' + t['abbrev']
                    modApprovalNeeded = serversList[str(message.guild.id)].get('tradeapproval', 'off') == 'on'
                    serversList[str(message.guild.id)]['openTrades'].append({
                        "tradeData": tradeData,
                        "confirmationNeeded": confirmTeams,
                        "modApprovalNeeded": modApprovalNeeded,
                        "tradeTids": [teamOneTid, teamTwoTid],
                        "season": export['gameAttributes']['season'],
                        "phase": export['gameAttributes']['phase']
                    })
                    confirmText = f'To confirm this trade, confirmation is required from the following team(s):{teamsText}'
                    if modApprovalNeeded:
                        confirmText += ' and a **mod**'
                    confirmText += '\n' + "*To confirm, simply type 'confirm' in the channel.*"
                    embed.add_field(name='Confirmation', value=confirmText, inline=False)
...
async def confirm_message(message):
    ...
    confirmedTrade = None
    isMod = message.author.guild_permissions.manage_messages
    try:
    ...
    for o in openTrades:
        if authorTid in o['confirmationNeeded']:
            confirmedTrade = o
        elif isMod and o.get('modApprovalNeeded', False) and authorTid not in o.get('tradeTids', []):
            confirmedTrade = o
    ...
    if confirmedTrade == None:
        ...
        # Determine what this confirm covers
        gmConfirmed = False
        modConfirmed = False
        if authorTid in confirmedTrade['confirmationNeeded']:
            confirmedTrade['confirmationNeeded'].remove(authorTid)
            gmConfirmed = True
        if isMod and confirmedTrade.get('modApprovalNeeded', False) and authorTid not in confirmedTrade.get('tradeTids', []):
            confirmedTrade['modApprovalNeeded'] = False
            modConfirmed = True

        # Check what's still needed
        stillNeedTeams = confirmedTrade['confirmationNeeded']
        stillNeedMod = confirmedTrade.get('modApprovalNeeded', False)

        if stillNeedTeams or stillNeedMod:
            # Build status message
            remaining = []
            for tid in stillNeedTeams:
                for t in teams:
                    if t['tid'] == tid:
                        remaining.append(t['abbrev'])
            if stillNeedMod:
                remaining.append('mod approval')
            parts = []
            if gmConfirmed:
                parts.append('GM confirmed')
            if modConfirmed:
                parts.append('Mod approved')
            text = f"✅ **{' & '.join(parts)}.** Still awaiting: {', '.join(remaining)}."
            ...
            # Trade analysis AI media removed

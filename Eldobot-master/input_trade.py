...
async def input_trade(export, tradeData, message,actuallyinput = True):
    ...
    tradeData = {int(k): v for k, v in tradeData.items()}
    ...
    if actuallyinput:
        for tid, otherTid in tradeTeams:
            if tid is None or tid not in tradeData:
                continue
            ...
            for a in assets:
                ...
                if a['type'] == 'player':
                    ...
                    assetDict = {
                        ...
                        "contract": matchedPlayer['contract']
                    }
                if a['type'] == 'draftPick':
                    pickSeason = None
                    pickRound = None
    ...
    #update export
    if actuallyinput:
        await basics.save_db(export, data_path(f'exports/{message.guild.id}-export.json'))
    return newEvent['eid']
